...
export async function getCategoryList(): Promise<Category[]> {
	...
	try {
		...
		// 添加文章中的分类
		categoryCountMap.forEach((_count, name) => {
		...
		const categories: Category[] = Array.from(allCategoryNames)
			.map((name) => {
				// 使用slug系统生成URL
...
export async function getPostsByCategory(
	...
	try {
		...
		if (!actualCategoryName) {
			...
			logger.info(
				`🔄 通过映射找到分类名称: "${categorySlugOrName}" -> "${actualCategoryName}"`,
			);
		}
		...
		logger.success(
			`✅ 获取到分类 ${actualCategoryName} 的 ${categoryPosts.length} 篇文章`,
		);
...
// 获取分类的布局类型
export async function getCategoryLayoutType(
	categoryIdentifier: string,
): Promise<"grid" | "masonry"> {
	console.log(
		`🎨 getCategoryLayoutType 被调用，分类标识: ${categoryIdentifier}`,
	);

	try {
		// 尝试从Strapi获取分类信息
		const { getCategoryByName, getCategoryBySlug } = await import(
			"../lib/strapi"
		);
