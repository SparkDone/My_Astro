...
export function removeDuplicateStyles(html: string): string {
	...
	duplicateStyles.forEach((duplicateStyle) => {
		...
		const secondOccurrence = optimizedHtml.indexOf(
			duplicateStyle,
			firstOccurrence + 1,
		);
...
export function generateOptimizationReport(
	...
	// 检测应用的优化
	if (
		originalHtml.includes("debug-info") &&
		!optimizedHtml.includes("debug-info")
	) {
		optimizations.push("移除调试内容");
	}

	if (
		originalHtml.match(/<style[^>]*>[\s\S]*?<\/style>/g)?.length !==
