		...
		containers.forEach((container) => {
			...
			if (currentLayout !== this.currentLayout) {
				...
				if (
					...
					console.log(
						`🔄 容器布局不匹配: ${currentLayout} -> ${this.currentLayout}`,
					);
				}
			}
		});
...
// 全局初始化函数
window.initUniversalLayoutSwitcher = (options = {}) => {
...
// 调试函数
window.debugLayoutSwitcher = () => {
	console.log("🔍 布局切换器调试信息:");
	console.log("- 实例存在:", !!window.universalLayoutSwitcherInstance);
	console.log(
		"- 按钮数量:",
		document.querySelectorAll(".layout-switch-btn").length,
	);
	console.log(
		"- 容器数量:",
		document.querySelectorAll(".universal-post-list").length,
	);
	console.log(
		"- 当前布局:",
		window.universalLayoutSwitcherInstance?.currentLayout,
	);
...
window.forceReinitLayoutSwitcher = () => {
	...
	if (switcher) {
		const defaultLayout =
			switcher.getAttribute("data-default-layout") || "grid";
		const targetSelector =
			switcher.getAttribute("data-target-selector") || ".universal-post-list";
...
// 调试函数
window.debugUniversalLayoutSwitcher = () => {
	...
	console.log(
		"是否已初始化:",
		window.universalLayoutSwitcherInstance?.isInitialized,
	);
	console.log(
		"目标选择器:",
		window.universalLayoutSwitcherInstance?.targetSelector,
	);
...
// 强制切换函数
window.forceUniversalLayoutSwitch = (layout) => {
