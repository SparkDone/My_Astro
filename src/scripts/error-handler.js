...
export function initializeErrorHandler(options = {}) {
	const { suppressStrapiErrors = true, enableLogging = true } = options;
	...
	window.addEventListener("error", (event) => {
		...
		if (shouldSuppressError(errorMessage, errorName)) {
			if (enableLogging) {
				console.warn(
					"🔐 Suppressed Strapi error from displaying to user:",
					errorMessage,
				);
			}
	...
	window.addEventListener("unhandledrejection", (event) => {
		...
		if (shouldSuppressError(errorMessage, errorName)) {
			if (enableLogging) {
				console.warn(
					"🔐 Suppressed Strapi promise rejection from displaying to user:",
					errorMessage,
				);
			}
			event.preventDefault();
