...
export function adaptStrapiArticle(strapiArticle: StrapiArticle): PostEntry {
	...
	return {
		...
		render: async () => {
			...
			const finalHtml = htmlContent.startsWith("<")
				? htmlContent
				: `<p>${htmlContent}</p>`;
...
export function adaptStrapiArticles(
	strapiArticles: StrapiArticle[],
): PostEntry[] {
	return strapiArticles.map(adaptStrapiArticle);
}

// 为文章添加前后导航信息
export async function addNavigationInfo(
	articles: PostEntry[],
): Promise<PostEntry[]> {
...
// 获取分类列表和计数
export function getCategoriesFromArticles(
	articles: PostEntry[],
