...
export class ContentManager {
	...
	async getPostBySlug(slug: string): Promise<PostEntry | undefined> {
		const cacheKey = generateCacheKey("post-by-slug", {
			slug,
			useStrapi: this.useStrapi,
		});
	 ...
	 * 从Strapi获取文章（带降级处理）
	 */
	private async getStrapiPostsWithFallback(
		cacheKey: string,
	): Promise<PostEntry[]> {
	 ...
	 * 实际从Strapi获取单篇文章
	 */
	private async fetchStrapiPostBySlug(
		slug: string,
	): Promise<PostEntry | undefined> {
		return withErrorBoundary(
			async () => {
...
export const getPostBySlug = (slug: string) =>
	contentManager.getPostBySlug(slug);
...
export const getPostsByCategory = (category: string) =>
	contentManager.getPostsByCategory(category);
