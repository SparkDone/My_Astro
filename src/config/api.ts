...
export const config = {
	...
	features: {
		useStrapi:
			import.meta.env.USE_STRAPI === "true" ||
			process.env.USE_STRAPI === "true",
		useHybridMode:
			import.meta.env.USE_HYBRID_MODE === "true" ||
			process.env.USE_HYBRID_MODE === "true",
...
if (config.development.enableDebugLogs) {
	...
	console.log(
		`   - 缓存TTL: ${config.cache.ttl}ms (${config.cache.ttl / 1000}秒)`,
	);
